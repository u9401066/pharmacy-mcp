...
class TFDAClient:
    ...
    async def _fetch_drug_permits(
        ...
        # Fetch from API
        url = (
            self.ACTIVE_PERMITS_JSON_URL if active_only else self.DRUG_PERMITS_JSON_URL
        )
    ...
    async def search_drug_by_name(
        ...
        for drug in data:
            ...
            if (
                ...
                or query_lower in english_name
    ...
    async def get_drug_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "dosage_form_distribution": dict(
                sorted(dosage_forms.items(), key=lambda x: x[1], reverse=True)[:20]
            ),  # Top 20 dosage forms
        }
...
def translate_drug_name(
    name: str, to_language: str = "chinese"
) -> dict[str, Any] | None:
    """
